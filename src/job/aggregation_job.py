# Job 2 - Fixed time window aggregation
# relies on Job 1's kafka sink process_events_kafka

# reads event from kafka source containing IP, host, referrer, URL, and geo-location.
# aggregates events using 5-minute tumbling windows
#   - Counts hits per `host` and writes to `processed_events_aggregated` in PostgreSQL.
#   - Counts hits per `host` and `referrer` and writes to `processed_events_aggregated_source` in PostgreSQL.
# - Writes the aggregated results to PostgreSQL using JDBC.
